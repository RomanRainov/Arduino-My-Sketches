...
void loop() {
  ...
  if (speedMove > 0) {
    ...
    if (channel2Value > 0) {
      ...
      motor.forward(speedLeft, speedRight - 100);
    } else {
      motor.backward(speedLeft, speedRight - 100);
    }
  } else if (abs(channel1Value) >= CH1_MIN) {
    if (channel1Value > 0) {
      motor.rotateRight(abs(channel1Value) + 100);
    } else {
      motor.rotateLeft(abs(channel1Value) + 100);
    }
  } else {
    motor.stop();
  }
